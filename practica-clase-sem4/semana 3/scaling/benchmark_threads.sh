...
executable="./matmul_tiled_openmp"
data_file="matmul_threads.dat"
plot_file="matmul_threads.png"

if [[ ! -x "$executable" ]]; then
  make matmul_tiled_openmp
fi
...
if ! command -v gnuplot >/dev/null 2>&1; then
  printf 'Benchmark terminado: %s\n' "$data_file"
  printf 'No se genero %s porque gnuplot no esta instalado.\n' "$plot_file" >&2
  exit 2
fi
...
set title "Matmul tiled OpenMP: tiempo por numero de threads"
set xlabel "Threads"
