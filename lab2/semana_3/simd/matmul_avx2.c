...
void transpose_matrix_1024(const float matrix[MATRIX_SIZE][MATRIX_SIZE],
...
void matrix_multiply_avx2_1024(const float a[MATRIX_SIZE][MATRIX_SIZE],
...
static void init_matrix_1024(float matrix[MATRIX_SIZE][MATRIX_SIZE],
                             float scale) {
    for (int row = 0; row < MATRIX_SIZE; ++row) {
        for (int col = 0; col < MATRIX_SIZE; ++col) {
...
static double checksum_matrix_1024(
...
int main(int argc, char **argv) {
    ...
    double operations =
        (double)repetitions * 2.0 * MATRIX_SIZE * MATRIX_SIZE * MATRIX_SIZE;
